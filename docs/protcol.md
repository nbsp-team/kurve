...
Ответы сервера - в формате JSON. При успешной обработке запроса поле **error** содержит значение **null**. При возникновении ошибки в поле **error** содержится объект с кодом ошибки и его текстовым описанием. Если поле **error** не равно **null**, то поля **response** может не существовать, иначе оно должно обязательно присутствовать.
